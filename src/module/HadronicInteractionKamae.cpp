...
double HadronicInteractionKamae::PhotonDiff(double Esec, double Tp) const {
    ...
    if(Tp > 5.52 * GeV) {
        ...
        b[1] = 35.22 + 3.8026 * std::tanh(-2.5976 * (y + 1.9)) - 2.1870e-4 * pow_integer<2>(y + 369.13);
...
double HadronicInteractionKamae::Photon1232(double Esec, double Tp) const {
    ...
    c[4] = -0.728074 - 0.48828 * y - 0.092876 * y2;
...
double HadronicInteractionKamae::Photon1600(double Esec, double Tp) const {
    ...
    d[3] = -1.50678 - 0.87211 * y - 0.17097 * y2;
...
double HadronicInteractionKamae::ElectronDiff(double Esec, double Tp) const {
    ...
    b[6] = 1.4862 + 0.995244 * y - 0.042763 * y2 - 0.0040065 * y3 + 0.0057987 * y4;
...
double HadronicInteractionKamae::PositronND(double Esec, double Tp) const {
    ...
    // table 5
    z = y + 3.3;
    a[0] = z * (-0.79606 + z * (7.7696 + z * (-3.9326 + z * (0.80202 - 0.054994 * z))));
    ...
    a[7] = 138.26 - 539.84 * log10(0.12467 * (y + 3.9)) - 1.9869e4 / pow_integer<2>(y + 7.+884) + 1.0675 * y2;
...
double HadronicInteractionKamae::PositronDiff(double Esec, double Tp) const {
    ...
    b[7] = 3.0551 + 3.5241 * tanh(-0.36739 * (y + 2.1)) - 0.13382 * (y - 2.7718);
...
double HadronicInteractionKamae::Positron1600(double Esec, double Tp) const {
    ...
    d[4] = -1.297 - 0.13647 * y - 0.41197 * y2 - 0.10641 * y3;
...
double HadronicInteractionKamae::ElectronNeutrinoDiff(double Esec, double Tp) const{
    ...
    if(Tp > 11.05 * GeV) {
        ...
        b[2] = -17.231 + 0.041100 * tanh(7.9638 * (y + 1.9)) - 0.0554499 * y + 2.5866e-4 * pow_integer<2>(y + 1.8998);
        z = y + 1.8998;
        b[3] = 12.335 - 12.893 * exp(-1.4412 * pow_integer<2>(z / (1. + 5.5969 * z)));
    }
    z = y + 2.2;
    b[4] = 1.3558 + 0.46601 * (y + 2.95) + 0.52978 * z * z + 0.79575 * exp(-5.4007 * pow_integer<2>(z + 4.6121 * z * z));
...
double HadronicInteractionKamae::AntiElectronNeutrinoND(double Esec, double Tp) const {
    ...
    a[5] = 4.7673e-5 + 5.4936e-5 * log10(0.34352 * (y + 4.3)) + 0.00020740 / (y + 4.9772);
...
double HadronicInteractionKamae::MuonNeutrinoND(double Esec, double Tp) const {
    ...
    // table 8 
    z = y + 3.3;
    a[0] = z * (-0.63611  + z * (9.9015 + z * (-4.5897 + z * (0.01778 + z * - 0.060724))));
    ...
    a[6] = -267.55 - 0.21018 * log10(0.016793 * (y + 3.9)) + 69.586 * y - 9.9930 * y2;
    a[7] = 2742.8 + 222.01 * log10(9.7401 * (y + 3.9)) - 4772.5 / (y + 19.773) - 6.1001 * y2;
    a[8] = -0.11857 + 0.39072 * y - 0.037873 * y2 + 0.0022265 * y3 + 0.0046931 * y4;
...
double HadronicInteractionKamae::MuonNeutrinoDiff(double Esec, double Tp) const {
    ...
    if(Tp > 11.05 * GeV) {
        ...
        b[2] = -1.4978 - 0.58163 * tanh(-0.36488 * (y + 1.8)) + 0.031825 * (y + 2.8097) + 0.022796 * pow_integer<2>(y - 1.8861);
        z = y + 3.8835;
...
double HadronicInteractionKamae::MuonNeutrino1232(double Esec, double Tp) const {
    ...
    c[2] = -6.2458 - 6.2939 * tanh(21.592 * (y + 2.1)) + 0.37440 * y;
    c[3] = 2.7485 + 1.1692 * y;
...
double HadronicInteractionKamae::MuonNeutrino1600(double Esec, double Tp) const {
    ...
    // table 8
    d[0] = 2.5489 * exp(-58.488 * pow_integer<2>(z / (1. + 0.3154 * z))) - 0.83039 - 0.34412 * y;
    d[1] = 88.173 + 65.148 * y + 12.585 * y2;
    d[2] = -7.0962 - 7.1690 * tanh(30.89 * (y + 2.1)) +  0.38032 * y;
    d[3] = 2.2624 + 1.1806 * y - 0.0043450 * y2 - 0.04302 * y3;
...
double HadronicInteractionKamae::AntiMuonNeutrinoDiff(double Esec, double Tp) const {
    ...
    if(Tp > 11.05 * GeV) {
        ...
        z = y + 2.219;
        b[1] = -8.1145 + 7686 * exp(2.4046e4 * pow_integer<2>(z / (1. + 81.105 *z)));
